...
SliderUnlock.prototype.handerOut = function () {
    ...
    //_self.move();
    if (_self.index < _self.threshold) {
        _self.reset();
    }
};
...
SliderUnlock.prototype.move = function () {
    var _self = this;
    if ((_self.index + 0) >= _self.threshold) {
        _self.index = _self.threshold - 0;
        //停止
        _self.swipestart = false;
        //解锁
        _self.isOk = true;
    ...
    if (_self.index == _self.threshold && _self.max > 0 && _self.isOk) {
        _self.success();
    }
...
SliderUnlock.prototype.reset = function () {
    ...
    _self.elm.find(".slideunlock-image-piece").animate({left: _self.index}, _self.opts.duration);

    _self.elm.find(".slideunlock-label").animate({left: _self.index}, _self.opts.duration)
        .next(".slideunlock-label-tip").animate({opacity: 1}, _self.opts.duration);
    _self.updateView();
};
...
SliderUnlock.prototype.backgroundTranslate = function () {
    ...
    _self.elm.find(".slideunlock-image-piece").css("left", _self.index/2 + "px");

    _self.elm.find(".slideunlock-label").css("left", _self.index + "px")
        .next('.slideunlock-label-tip').css("opacity", 1-(parseInt($(".slideunlock-label").css("left"))/_self.max));
};
...
SliderUnlock.prototype.updateView = function () {
    ...
    if (_self.index == (_self.threshold - 0)) {
        ...
        _self.elm.addClass("success").find(".slideunlock-label-tip").html(_self.opts.successLabelTip).css(style);
        _self.elm.find(".slideunlock-label").remove();
    } else {
        $(".slideunlock-lockable").val(0);
        _self.elm.removeClass("success").find(".slideunlock-label-tip").html(_self.opts.labelTip);
    }
    _self.always();
};